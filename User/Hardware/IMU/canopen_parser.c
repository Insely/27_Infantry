...
static int parse_canopen_euler(const hipnuc_can_frame_t *frame, can_sensor_data_t *data)
{
    ...
    if (data->last_yaw > 2.0f && data->imu_yaw < -2.0f)
		data->yaw_cnt += ((PI - data->last_yaw) + (data->imu_yaw + PI));
	else if (data->last_yaw < -2.0f && data->imu_yaw > 2.0f)
		data->yaw_cnt -= ((PI + data->last_yaw) + (PI - data->imu_yaw));
	else
		data->yaw_cnt += (data->imu_yaw - data->last_yaw);
	data->last_yaw = data->imu_yaw;
...
/*
 * Function   : update_hipnuc_data
 */
void update_hipnuc_data(const FDCAN_RxHeaderTypeDef *rx,uint8_t*rx_data)
{
    hipnuc_can_frame_t f;
    f.can_id = (rx->Identifier & HIPNUC_CAN_SFF_MASK);
    f.can_dlc = rx->DataLength;
    memcpy(f.data, rx_data, (f.can_dlc < 8) ? f.can_dlc : 8);
    f.hw_ts_us = 0;
    hipnuc_can.node_id = hipnuc_can_extract_node_id(f.can_id);
    hipnuc_can.hw_ts_us = f.hw_ts_us;
    canopen_parse_frame(&f, &hipnuc_can);
